...
- **A message's own attachments don't appear until the next reload — including for the person who just sent it.** `postSnagUpdate`'s `revalidatePath` runs (and shows the new text bubble) before `attachDraftMedia` even starts uploading, since the two are separate calls — one a server action, one a direct client-side Supabase Storage/table write with nothing to trigger a second refresh once it finishes. Confirmed live: after posting a message with two photos, the text appeared immediately but the thumbnails were only visible after manually reloading the page. Noticed while testing multi-photo/video support (17 Aug 2026) but pre-dates it — the original single-photo/video compose box had the same gap, just less noticeable with one file. Fixing it needs an explicit `router.refresh()` (or equivalent) after `attachDraftMedia` resolves.
