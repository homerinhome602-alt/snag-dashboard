...
**The expanded row's chat panel uses the same trick one level down — as built (17 Aug 2026).** The `<td colSpan={13}>` it lives in can't itself be sticky (position:sticky doesn't work on a cell spanning the full row width), but a plain `<div>` nested inside that wide cell can be — it sticks to the left edge of the table's own scroll container regardless of how far the table is scrolled. Width is capped at `w-[min(1000px,90vw)]` so it reads as a normal-sized panel instead of stretching to match the (much wider) row it's nested in.
